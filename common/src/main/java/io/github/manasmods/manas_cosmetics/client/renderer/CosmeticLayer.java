...
public final class CosmeticLayer<T extends Player, M extends EntityModel<T>>
    ...
    private static void applyHandTransform(PoseStack ps, Player player, EntityModel<?> parentModel) {
        if (parentModel instanceof HumanoidModel<?> humanoid) {
            ...
            // Z=-125° gives blade direction (+0.819, -0.574, 0) in render space:
            // render -Y = world +Y (upward) and render +X = screen-right (from behind).
            // Axial roll rotates 90° around the blade's own axis (sign*0.819, -0.574, 0)
            // so the edge faces up/down rather than left/right, without disturbing the tilt.
